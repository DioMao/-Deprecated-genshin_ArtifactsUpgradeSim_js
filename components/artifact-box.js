    ...
    <div class="main-container">
        ...
        <div class="selectBox d-flex">
            <select class="form-select" aria-label="Default select example" v-model="userSetting.filterPart">
                ...
                <option :value="part" v-for="part in artiConst.val.parts">{{ toChinese(part,"parts") }}</option>
            </select>
        </div>
        <div class="container-fluid demo-container" ref="scrollListener">
            ...
            <div v-for="(Artifacts,index) in ArtifactsList" :ref="'artifact-'+'index'" :id="'artifact-'+index" class="ArtifactsBox card rounded shawdow-sm" :class="(Artifacts.symbol===showSymbol?'isSelect':'')" v-show="(userSetting.filterPart==='default' || userSetting.filterPart === Artifacts.part) && (userSetting.filterMain === 'default' || userSetting.filterMain === Artifacts.mainEntry)" @click="changeShowSymbol(Artifacts.symbol)">
                <div class="card-body ArtifactsTitle" :style="{backgroundImage:'url(./img/A-'+ Artifacts.part + '.png)'+(Artifacts.symbol===showSymbol?',url(./img/genshin-symbol.png)':'')}">
                    <div class="islock" v-if="Artifacts.lock">
                        <svg t="1631861008451" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4168" width="0.5rem" height="0.5rem" fill="rgb(255,138,117)">
                    ...
                    <a id="mobileShow" data-bs-toggle="offcanvas" href="#offcanArtifactShow" aria-controls="offcanArtifactShow">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1rem" height="1rem" fill="#fff" class="bi bi-info-circle-fill" viewBox="0 0 16 16">
            ...
            <div class="ArtifactsBox card rounded" v-for="i in fillCount" style="visibility:hidden;"></div>
        </div>
        <div class="ArtifactShowBox">
            <artifact-show @upgrade="ArtifactUpgrade" @init="initArtifact" @del="deleteArtifact" @lock="lockChange" :showdetail="showDetail" :index="showIndex" v-if="showIndex>=0"></artifact-show>
        ...
        <footer>
            <div class="gap-2 d-md-flex justify-content-end buttonBox clearfix">
                ...
                <svg t="1631795704220" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="719" width="1.25rem" height="1.25rem">
                ...
                <button id="sort" class="btn me-auto btn-genshin" data-bs-toggle="dropdown" aria-expanded="false">
                    <svg class="bi bi-funnel-fill" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem" fill="currentColor">
    ...
    methods: {
        ...
        setRadarChart() {
            ...
            let radarChart = echarts.init(this.$refs.radarChartBox),
                radarChart2 = echarts.init(this.$refs.radarChartBox2),
                entryValue = artiConst.val.entryValue;
